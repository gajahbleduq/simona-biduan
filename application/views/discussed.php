            ...
            <div class="card-body">
                <div class="content-header row">
                    <div class="content-header-left col-md-9 col-12 mb-2">
                        <div class="row breadcrumbs-top">
                            <div class="col-12">
                                <h2 class="float-start mb-0">Daftar Tiket Diskusi</h2>
                            </div>
                        </div>
                    </div>
                    <form action="<?= site_url('discussed'); ?>" method="POST">
                        ...
                        <table class="datatables-basic table" id="datatable1">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $pic; ?></td>
                                    <td>
                                        <span id="spanJumlahDiskusi<?=$id;?>" onclick="showModalViewDiskusi(this)"
                                            data-id="<?=$id;?>" data-noTicket="<?=$noticket;?>"
                                            data-isFinished="<?=$is_finished;?>" data-fromView="TabelJumlahDiskusi"
                                    ...
                                    <td>
                                        ...
                                        <div class="d-flex justify-content-center align-content-center gap-1 mt-1">
                                            <button class="btn btn-sm btn-secondary rounded"
                                                onclick="showModalTambahDiskusi(this)" data-id="<?=$id;?>"
                                                data-noTicket="<?=$noticket;?>" data-bs-toggle="tooltip"
                                                data-bs-placement="bottom" title="Tambah Diskusi">
            ...
            <div class="modal-body pb-5 px-sm-5 pt-50">
                ...
                <div class="row">
                    ...
                    <form id="formChangeStatus" class="row gy-1 formChangeStatus" method="POST"
                        ...
                        <input type="hidden" id="urlfrom" name="urlfrom" value="discussed" />
                        <input type="hidden" id="to" name="to" value="6" />
...
<div class="modal fade" id="modalTambahDiskusi" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-edit-status">
        <div class="modal-content">
            <div class="modal-header bg-transparent">
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            ...
            <div class="modal-body pb-5 px-sm-5 pt-50">
                <div class="text-center mb-2">
                    <h1 class="mb-1">Tambah Diskusi</h1>
                    <h4>No Tiket: <span id="spanNoTicketDiskusi"></span></h4>
                </div>
                <div class="row">

                    <form id="formTambahDiskusi" class="row gy-1 formTambahDiskusi" method="POST"
                        enctype="multipart/form-data" autocomplete="off">
                        <input type="hidden" id="idReportDiskusi" name="idReportDiskusi" class="form-control" />
                        <input type="hidden" id="fromDiskusi" name="fromDiskusi" value="5" />
                        <div class="col-12 mb-1">
                            <label class="form-label requiredField" for="add_diskusi">Judul Diskusi</label>
                            <input type="text" id="add_diskusi" name="add_diskusi" class="form-control" required />
                            <div class="invalid-feedback">Mohon masukkan judul diskusi.</div>
                        </div>
                        <div class="col-12 mb-1">
                            <label class="form-label" for="description_diskusi">Deskripsi</label>
                            <textarea name="description_diskusi" id="description_diskusi"
                                class="form-control"></textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label" for="tanggal_diskusi">Tanggal Diskusi</label>
                            <input type="text" class="form-control dateDiskusi flatpickr-date-time"
                                name="tanggal_diskusi" id="tanggal_diskusi" />
                        </div>
                        <div class="col-12 mb-1">
                            <div id="section_file_diskusi">
                                <div style="padding: 0; padding-bottom: 0px;">
                                    <div class="row">
                                        <div class="col-lg-10">
                                            <div class="mb-1">
                                                <div>
                                                    <label class="form-label" for="add_file_diskusi">Nama File</label>
                                                    <input type="text" class="form-control" name="add_file_diskusi[]" />
                                                </div>
                                            </div>
                                            <div class="mb-1">
                                                <div>
                                                    <label class="form-label" for="uploadfilediskusi">Unggah</label>
                                                    <input type="file" class="form-control" name="uploadfilediskusi[]"
                                                        accept=".gif, .jpg, .png, .doc, .docx, .pdf" />
                                                    <p><small class="text-muted">Max Size 2Mb , Type: .gif, .jpg, .png,
                                                            .doc, .docx, .pdf</small></p>
                                                </div>
                        ...
                        <div class="row">
                            <div class="col-12">
                                <button class="btn btn-icon btn-primary" type="button" onclick="addRowFileDiskusi()">
                                    <i data-feather="plus" class="me-25"></i>
                                    <span>Tambah File</span>
                                </button>
                            </div>
                        ...
                        <div class="col-12 text-center mt-2 pt-50">
                            <button type="button" onclick="submitformdiskusi()"
                                class="btn btn-primary me-1">Simpan</button>
                            <button type="reset" class="btn btn-outline-secondary" data-bs-dismiss="modal"
                                aria-label="Close">
                                Batalkan
                            </button>
...
function showModalFinish(a) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            window.location.href = "<?= site_url('finish_ticket/')?>" + id + "/" + noTicket + "/discussed";

        }
    });
}

function showModalTambahDiskusi(a) {
    // Picker
    var picker = $('.dateDiskusi');
    picker.flatpickr({
        allowInput: true,
        enableTime: true,
    ...
    $('#idReportDiskusi').val(id);
    $('#spanNoTicketDiskusi').html(noTicket);

    if ($(".div-entry-diskusi")[0]) {
        $("div.div-entry-diskusi").remove();
    }
...
function addRowFileDiskusi() {
    var id = $('#idReportDiskusi').val();

    var div = `
            <div class="div-entry-diskusi" id="div-entry-diskusi` + id + `" style="padding: 0; padding-bottom: 0px">
                <div class="row">
                    <div class="col-lg-10">
						<label class="form-label" for="file-name">Nama File</label>
						<input type="text" class="form-control" name="add_file_diskusi[]" />
						<label class="form-label" for="upload">Unggah</label>
						<input type="file" class="form-control" name="uploadfilediskusi[]" accept=".gif, .jpg, .png, .doc, .docx, .pdf" />
						<p><small class="text-muted">Max Size 2Mb , Type: .gif, .jpg, .png, .doc, .docx, .pdf</small></p>
                    </div>
                    <div class="col-lg-2" style="display: flex; align-items: center">
                        <button type="button" class="btn btn-outline-danger text-nowrap px-1"
								onclick="removeRowFileDiskusi(` + id + `)">
                            <i data-feather="trash-2" class="me-25"></i>
                        </button>
                    </div>
                </div>
    ...
    $('#section_file_diskusi').append(div);
...
function removeRowFileDiskusi(id) {
    var divToRemove = document.getElementById("div-entry-diskusi" + id);
    divToRemove.parentNode.removeChild(divToRemove);
}

function submitformdiskusi() {

    var formTambahDiskusi = $('.formTambahDiskusi');

    // Bootstrap Validation
    // --------------------------------------------------------------------
    var checkvalidity = true;
    if (formTambahDiskusi.length) {
        Array.prototype.filter.call(formTambahDiskusi, function(form) {
            if (form.checkValidity() === false) {
                checkvalidity = false;
                form.classList.add('invalid');
            }
    ...
    if (checkvalidity) {
        ...
        Swal.fire({
            title: 'Apakah anda yakin?',
            text: 'Akan menambahkan data diskusi',
            icon: "question",
            showCancelButton: true,
            confirmButtonText: "Ok, saya yakin!",
            cancelButtonText: 'Tidak, batalkan',
            ...
            if (result.isConfirmed) {
                ...
                var form = $("#formTambahDiskusi");
                var formData = new FormData(form[0]);

                $.ajax({
                    url: "<?= site_url('ajax/tambah_diskusi'); ?>",
                    type: 'POST',
                    data: formData,
                    success: function(data) {
                        Swal.close();
                        ...
                        if (dataParse.success) {
                            title = 'Success';
                            icon = 'success';
                            var id = dataParse.data.id;
                            var idspanjumlahdiskusi = $('#spanJumlahDiskusi' + id);
                            var txbJumlahDiskusi = $('#txbJumlahDiskusi' + id);
                            var jumlahdiskusi = parseInt(txbJumlahDiskusi.val()) + 1;
                            txbJumlahDiskusi.val(jumlahdiskusi);
                            idspanjumlahdiskusi.html(jumlahdiskusi);
                        }
...
function removeDiskusi(a) {
    var idReport = $(a).attr('data-idReport');
    var idItem = $(a).attr('data-idItem');
    var idName = $(a).attr('data-idName');
    var txbJumlahDiskusi = $('#txbJumlahDiskusi' + idReport);
    var jumlahDiskusi = parseInt(txbJumlahDiskusi.val());

    Swal.fire({
        title: 'Mohon tunggu !',
        html: 'Menghapus data diskusi ....',
        allowOutsideClick: false,
        showConfirmButton: false
    });
    ...
    Swal.fire({
        title: 'Apakah anda yakin?',
        text: 'Akan menghapus diskusi ini',
        icon: "question",
        showCancelButton: true,
        confirmButtonText: "Ok, saya yakin!",
        cancelButtonText: 'Tidak, batalkan',
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "<?= site_url('ajax/remove_diskusi/'); ?>" + idItem,
                type: "POST",
                success: function(data) {
                    ...
                    if (dataParse.success) {
                        ...
                        jumlahDiskusi = jumlahDiskusi - 1;
                        $('#spanJumlahDiskusi' + idReport).html(jumlahDiskusi);
                        $(txbJumlahDiskusi).val(jumlahDiskusi);
                        $("#" + idName).remove();
